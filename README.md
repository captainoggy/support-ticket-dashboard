...
- **TanStack Query is the whole data layer.** Server state is cached with a short stale time, so moving between the list, board, and detail views is instant and revalidates in the background; the list keeps the previous page visible while the next loads. Writes update the cache optimistically and then invalidate related queries, so every view stays consistent and the server remains the source of truth. Realtime works by having the WebSocket layer invalidate the same cache, so there is never a second, divergent copy of the data. HTTP-level caching is deliberately not used: ticket data is per-user and mutable, so it belongs in the query cache, not in `Cache-Control`.
...
- **The API runs with `tsx` in the container** rather than a precompiled bundle: one less build pipeline in a timeboxed exercise. The web app *is* production-built (served by nginx in the Docker stack, and by Vercel's CDN in the live deploy).
...
- Remove the free-tier cold start by keeping the API warm (a paid instance or a scheduled ping) and add a custom domain
